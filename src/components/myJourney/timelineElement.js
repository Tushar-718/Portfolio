let timelineElements=[
    {
        id:1,
        title:"Web Developer Trainee",
        company:"Magic Edtech",
        location:"Noida",
        date:"Nov 2022 - Sept 2023",
        desc:`Assisted in the development of E-books.  Developing E-books for the client and integrating them with animations, games and interactivity to make book more lively.`,
        // desc:["Assisted in the development of E-books.","Responsible for handling page layout of ebooks from scratch. ","Developing E-books for the client and integrating them with animations, games and interactivity to make book more lively."],
        icon:"work"
    },
    {
        id:2,
        title:"Bachelors Of Technology in Information Technology",
        company:"Maharaja Surajmal Institute of Technology",
        location:"Delhi",
        date:"Aug 2018 - Jun 2022",
        desc:"Percentage : 80.09 %",
        // desc:["Percentage : 80.09 %"],
        icon:"school"
    },
    {
        id:3,
        title:"Class XII",
        company:"Kendriya Vidyalaya Andrews Ganj",
        location:"Delhi",
        date:"Apr 2017 - Jun 2018",
        desc:"Percentage : 70.80 %",
        // desc:["Percentage : 70.80 %"],
        icon:"school"
    },
    {
        id:4,
        title:"Class X",
        company:"Kendriya Vidyalaya Andrews Ganj",
        location:"Delhi",
        date:"Jun 2015 - May 2016",
        desc:"Percentage : 81.70 %",
        // desc:["Percentage : 81.70 %"],
        icon:"school"
    },
]
export default timelineElements;